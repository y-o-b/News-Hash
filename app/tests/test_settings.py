...
def test_source_errors_can_be_acknowledged_without_resetting_total(tmp_path) -> None:
    manager = SettingsManager(data_dir=tmp_path)
    manager.record_source_error("alpha", "Timeout", "fetch")
    manager.record_source_error("alpha", "Bad response", "interpret")
    ...
    assert manager.unacknowledged_source_error_type_counts() == {("alpha", "fetch"): 1, ("alpha", "interpret"): 1}
    ...
    assert manager.unacknowledged_source_error_type_counts() == {}
    assert manager.acknowledge_source_errors("missing") is False

    manager.record_source_error("alpha", "New error", "fetch")
    assert manager.unacknowledged_source_errors() == {"alpha": ["New error"]}
    assert manager.unacknowledged_source_error_type_counts() == {("alpha", "fetch"): 1}
